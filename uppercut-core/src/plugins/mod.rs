//! Sandboxed WASM frame-effect plugins (wasmtime). See docs/plugin-api.md.
...
/// Host that can run loaded plugins against RGBA frames.
...
impl PluginHost {
    ...
    pub fn add_dir(&mut self, root: &Path) -> Result<(), String> {
        ...
        let has_process = module.exports().any(|e| e.name() == "process");
        if !has_memory || !has_process {
            return Err("plugin must export `memory` and `process`".into());
        }
